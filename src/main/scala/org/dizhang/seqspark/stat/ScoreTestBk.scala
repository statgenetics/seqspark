...
import org.dizhang.seqspark.stat.ScoreTestBk._
...
object ScoreTestBk {
  ...
  def apply(binaryTrait: Boolean,
            ...
            estimates: Option[DenseVector[Double]]): ScoreTestBk = {
    x match {
  ...
  def apply(binary: Boolean,
            ...
            estimates: DenseVector[Double]): ScoreTestBk = {
    if (binary)
...
sealed trait ScoreTestBk extends HypoTest {
  val u: Double
  val v: Double
...
case class BinaryNoCovUniScoreTest(y: DenseVector[Double],
                                   x: DenseVector[Double]) extends ScoreTestBk {
...
case class BinaryCovUniScoreTest(y: DenseVector[Double],
                                 x: DenseVector[Double],
                                 cov: DenseMatrix[Double],
                                 estimates: DenseVector[Double]) extends ScoreTestBk {
  lazy val n = y.length
...
case class QuantNoCovUniScoreTest(y: DenseVector[Double], x: DenseVector[Double]) extends ScoreTestBk {
