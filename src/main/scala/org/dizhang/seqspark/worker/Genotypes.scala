...
object Genotypes {
  ...
  def statGdGq(self: Data[String])(ssc: SingleStudyContext): Unit = {
    ...
    val fm = first.format.split(":").zipWithIndex.toMap
    val gdi = fm("DP")
    val gqi = fm("GQ")
    val all = self.map(v =>
      v.toCounter(makeGdGq(_, gdi, gqi), new Int2IntOpenHashMap(Array(0), Array(1)))
        .reduceByKey(keyFunc))
  ...
  def makeGdGq(g: String, gdi: Int, gqi: Int): Int2IntOpenHashMap = {
    val fs = Genotype.Raw.fields(g)(gdi, gqi)
    if (g.startsWith(".")) {
      new Int2IntOpenHashMap(Array(0), Array(1))
    } else {
      val gd = General.insert(gdTicks, fs(0).toInt)
      val gq = math.min(fs(1).toDouble.toInt, 99) / 5
      val key = 20 * gd + gq
